...
export class TaskService {
    ...
    update(task: Task): Observable<Task> {
        ...
        // copy the feilds you need to modify, in this way to prevent
        // modify unnecessary parts
        const toUpdate = {
            desc: task.desc,
            priority: task.priority,
            dueDate: task.dueDate,
            reminder: task.remider,
            ownerId: task.ownerId,
            participantIds: task.participantIds,
            remark: task.remark
        };
        return this.http
        .patch(uri, JSON.stringify(task), {
            headers: this.headers
        })
        .map(res => res.json());
    }
    ...
    moveAll(srcListId: string, targetListId: string) : Observable<Task[]> {
        return this.get(srcListId)
            // get each task from task stream
            .mergeMap(tasks => Observable.from(tasks))
            // move each task to target list
            .mergeMap(task => this.move(task.id, targetListId))
            // merger target tasks into an array
            // [] is the inital array
            .reduce((arr, x) => [...arr, x], [])
    }
}